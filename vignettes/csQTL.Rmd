...
A `r Biocpkg("SummarizedExperiment")` object with bulk protein or gene expression contained in `counts` slot, and a "signature matrix" which serves as a reference of known cellular signatures contained as an element in `metadata` slot is required as input file. Note that the proteins or genes in signature matrix can be different from that in assay, but only common proteins or genes will be used to do devonvolution. 

In this current version, only `CIBERSORT` and `nnls` are supported as the deconvultion methods.
